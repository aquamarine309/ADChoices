...
export class DimBoost {
  static get power() {
    ...
    let boost = Effects.max(
      ...
      .timesEffectsOf(
        ...
        ChoiceGroup.reach100AM.choices.improveBoosts
  ...
  static multiplierToNDTier(tier) {
    const actualTier = ChoiceGroup.firstGalaxy.choices.boostForAllDim.canBeApplied ? 1 : tier;
    const normalBoostMult = DimBoost.power.pow(this.purchasedBoosts + 1 - actualTier).clampMin(1);
  ...
  static get unlockedByBoost() {
    ...
    if (
      ChoiceGroup.firstGalaxy.choices.boostForAllDim.canBeApplied ||
      boosts >= DimBoost.maxDimensionsUnlockable - 1
